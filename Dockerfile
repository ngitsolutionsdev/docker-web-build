...
# Mono/GitVersion
#
RUN apt install -y libgit2-24 git curl libcurl3 unzip mono-complete jq
WORKDIR /tmp
...
COPY gitversion /usr/bin/gitversion
...
RUN gitversion
RUN gitversion --get-var SemVer
RUN gitversion --get-var FullSemVer
